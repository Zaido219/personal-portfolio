      ...
      <div className="flex flex-col w-full p-3 rounded-sm gap-2 overflow-hidden">
        ...
        <div className="flex flex-row justify-between items-start w-full p-2 gap-4">
          <div className="flex-1 min-w-0">
            <h3 className="text-md md:text-sm font-bold text-black dark:text-zinc-300 tracking-wide break-words">
              {institutionName}
            </h3>
            <p className="text-xs md:text-xs text-slate-700 dark:text-zinc-300 mt-0.5 break-words">{location}</p>
          </div>

          {/* Timeline Badging - whitespace-nowrap keeps the date on one clean line */}
          <div className="text-xs md:text-xs font-semibold text-black dark:text-zinc-300 px-2.5 py-1 rounded-full shrink-0 whitespace-nowrap">
        ...
        {(degree || fieldOfStudy) && (
          <div className="text-md md:text-xs font-medium text-black dark:text-zinc-300 border-l-2 border-slate-400 dark:border-zinc-800 pl-3 my-1 break-words">
        ...
        {achievements.length > 0 && (
          <ul className="list-disc list-inside space-y-1.5 text-sm md:text-xs text-black dark:text-zinc-300 pl-1">
